...
router.post('/', apiKeyAuth, async (req: Request, res: Response) => {
  ...
  const code = await generateCode('RS');
  const pixCode = `000201HOTEL${code.replace(/-/g, '')}`;
  res.status(201).json(await prisma.hotelReservation.create({ data: { code, hotelId, roomId, guestName, email, phone, checkIn, checkOut, pixCode, pixLink: `pix://pay?code=${encodeURIComponent(pixCode)}` }, include: reservationInclude }));
});
